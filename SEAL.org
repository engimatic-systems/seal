...
credentials.  The broker exposes persistent operator-private session state at
=.seal-exec/runtime/=, sets =SEAL_OPERATOR_RUNTIME= to its absolute path, and
overrides =TMPDIR= beneath =world/=.  In =--bwrap= mode the private runtime is a
second writable bind beside the transported mailbox.  Neither mode restricts
network access, cloud authority, reads from the host, or writes made through
external APIs.
...
run:
  ...
  prepare persistent host-only .seal-exec/runtime
  ...
  launch parsed argv directly without shell re-parsing:
    ...
    explicit --bwrap mode:
      ...
      operator-private .seal-exec/runtime writable but not transported
...
strict argv proposal
  ...
  -> persistent operator runtime is writable through a distinct bind
