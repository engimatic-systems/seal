...
run (default):
  seal pull
  select newest ready command whose ID is not already claimed
    none -> report no command -> success
  move ready command to claimed
...
purge:
  seal pull so operator sees current ready entries
  require literal purge confirmation
  remove local ready directory
  construct fixed rsync plan from seal.toml
  sync an empty ready directory to peer ready with --delete
  leave other mailbox subtrees untouched
#+end_example
...
local peer with two ready commands
  purge with literal confirmation
  -> local ready removed
  -> peer ready emptied through scoped --delete
  -> peer world remains
