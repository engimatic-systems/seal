...
2. Require ordinary fixed flags, dry-run, configured rsync verbosity, secluded
   arguments, and one remote-shell executable token containing whitespace,
   quote, and backslash characters before its configured verbosity token.
...
** Exact reversed local push plan

Pseudocode:

1. Plan a local push through the same planner used by pull.
2. Require only the final mailbox operands to reverse: local source, peer
   destination.
