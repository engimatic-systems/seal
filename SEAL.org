...
=seal/config.odin=, =seal/init.odin=, =seal/transfer.odin=,
=seal/main_test.odin=, =seal/config_test.odin=, =seal/init_test.odin=, and
...
The command supplies the process environment. An ordinary run reports created
paths; a debug run also reloads the written configuration and reports its
effective values through the shared debug presentation.
...
2. Add the dry-run reporting profile and configured rsync verbosity only when
   requested.
3. For SSH, quote the pinned SSH path as one token in rsync's remote-shell
   command grammar, then add configured SSH verbosity as a separate token only
   in debug mode.
4. Give both mailbox roots a trailing slash; keep remote paths peer-side and
   local paths already resolved by configuration loading.
5. Add =--=, then order peer and local operands for pull or reverse the same
   operands for push.

#+begin_src odin :tangle seal/transfer.odin :name transfer-plan
...
2. On debug runs, report the shared effective configuration presentation.
3. Build one directional plan and invoke it once; dry-run changes only rsync
   argv.

#+begin_src odin :tangle seal/transfer.odin :name transfer-command
...
Only implemented actions exist. Later verbs are not represented.
...
3. Select =cfg=, =init PEER_PATH=, =pull=, =push=, help, or version for their
   exact spellings.
4. Accept =--ssh SSH_DESTINATION= only once and only after =init=; accept
   =--dry-run= only once and only after =pull= or =push=.
...
3. Dispatch =cfg= to read-only configuration display, =init= to local
   workspace creation, and =pull= or =push= to one directional process plan.
...
2. Require ordinary fixed flags, dry-run reporting flags, configured rsync
   verbosity, secluded arguments, and one remote-shell executable token
   containing whitespace, quote, and backslash characters before its configured
   verbosity token.
...
** Exact reversed local push dry-run plan

Pseudocode:

1. Plan a non-debug local push dry-run through the same planner used by pull.
2. Require the dry-run reporting profile without a debug verbosity flag.
3. Require the final mailbox operands to reverse: local source, peer
   destination.

#+begin_src odin :tangle seal/transfer_test.odin :name push-plan-tests
@(test)
