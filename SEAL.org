...
#+begin_example
parse global options + cfg | init | pull
...
pull -> typed transfer orchestration -> child outcome
...
failures. The transfer module owns only the structured pull plan and direct
child execution.
...
pull; it does not anticipate push.
...
if peer uses SSH:
  ...
  source = SSH destination + ":" + peer path with trailing slash
else:
  source = peer path with trailing slash

append rsync option terminator
append source
append local mailbox with trailing slash
...
*** Pull command adapter

Pull resolves configuration-relative endpoints and classifies failures only
where their operator context is known. Its error type is shared transfer
orchestration vocabulary rather than pull-specific policy.
...
construct pull process plan; classify failure as Planning
...
arguments. This pins both local and SSH plans, including debug-only verbosity,
dry-run placement, secluded arguments, the quoted pinned remote shell and its
option terminator, rsync's option terminator, direction, and trailing slashes.

Pseudocode:

1. Configure a local peer with =-vv= rsync verbosity and run a debug dry-run;
   require the exact child argv and structured debug plan.
2. Configure an option-shaped SSH destination with =-vvv= rsync and =-vv= SSH
   verbosity; require the exact child argv and structured debug plan, including
   the opaque destination, quoted executable, and remote-shell =--=.
