...
The parser trims horizontal whitespace and returns borrowed key/value slices.
It recognizes only blank, table, assignment, or invalid complete lines. Schema
names and configuration ownership remain outside this file.

Pseudocode:

1. Trim leading and trailing spaces and tabs; accept an empty result.
2. Recognize =[key]= only when the inner key has no whitespace and follows the
   ASCII key grammar.
3. Otherwise recognize one key, optional horizontal whitespace, ===, optional
   horizontal whitespace, and one complete double-quoted printable-ASCII value.
4. Reject comments, escapes, Unicode, control bytes, lists, and trailing bytes.

#+begin_src odin :tangle seal/config_syntax.odin :name config-syntax-complete-line-parser
