...
** Exact reversed push plans

Pseudocode:

1. Plan local and SSH pushes through the same planner used by pull.
2. Require the same fixed, dry-run, and debug arguments as the corresponding
   pull plans.
3. Require only the final mailbox operands to reverse: local source, peer
   destination.
