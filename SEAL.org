...
** Fields, policy, and resolution

Configuration stores semantic values only. =Config_State= exists only while
parsing to enforce duplicate and presence policy; SSH mode follows from a
nonempty validated =peer_ssh= value.

Pseudocode:

1. Recognize each physical line through the syntax file and reject invalid lines.
2. Accept exactly root =local_mailbox= plus peer/tools/debug tables and their
   known keys; reject duplicates and unknown names at the first line.
3. Require local mailbox, peer path, and absolute rsync; pair optional nonempty
   peer SSH with absolute tools SSH.
4. Accept debug scalars =""=, ="-v"=, ="-vv"=, or ="-vvv"= and default absent
   applicable fields to ="-v"=.
5. Resolve local paths from the selected configuration directory and retain an
   SSH peer path verbatim.

#+begin_src odin :tangle seal/config.odin :name config-schema
package main
