...
Parse/state destruction uses the allocator captured at allocation time.
...
2. Keep the backend table and parser allocator behind an opaque distinct handle;
   destroy the tree and state with that stored allocator.
