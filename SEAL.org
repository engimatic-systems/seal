...
The proposal contains one argv vector.  Shell execution remains available only
when the proposal names a shell explicitly.  =bwrap= gives the launched command
a read-only view of the host and a writable view of the whole mailbox.  It
inherits the broker's ambient environment, including =PATH= and credentials,
while overriding only =TMPDIR=.  It does not restrict network access, cloud
authority, reads from the host, or writes made through external APIs.  Operator
review is the command-safety decision in this experiment.
...
run:
  ...
  parse snapshot as exactly one nonempty argv array
    invalid:
      edit -> run vi outside sandbox -> snapshot + parse again
      other/EOF -> record invalid result -> seal push -> failure
  display numbered exact TOML + digest + indexed escaped argv
  ask operator:
  ...
  bwrap command:
    ...
    cwd mailbox/world; TMPDIR points beneath world; host /tmp stays read-only
