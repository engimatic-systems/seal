...
explicitly.  =bwrap= gives the launched command a read-only view of the host and
a writable view of the whole mailbox.  It inherits the broker's ambient
environment, including =PATH= and credentials, while overriding only =TMPDIR=.
It does not restrict network access, cloud authority, reads from the host, or
writes made through external APIs.  Operator review is the command-safety
...
run:
  ...
  display numbered exact TOML + digest + cwd resolution + indexed escaped argv
  ask operator:
  ...
  bwrap command:
    ...
    ambient broker environment inherited
    parsed argv executed directly without shell re-parsing
...
#+begin_example
fake seal + fake bwrap + strict argv proposal
  run with yes
  -> pull then push
