...
=command.sh= bytes, one launch, separated output streams, exit status, and a
host-local attempt record.

The command language remains arbitrary Bash.  =bwrap= gives the launched
command a read-only view of the host and a writable view of the whole mailbox.
...
run:
  ...
  require a regular, non-symlink command.sh
    invalid -> record invalid result -> seal push -> failure
  display numbered exact script + digest
  warn when script exceeds 40 lines
  ask operator:
    edit -> run vi outside sandbox -> redisplay and ask again
    yes  -> continue
    other/EOF -> record refusal -> seal push -> success
  copy approved command.sh into host-only attempt directory
  record started state before launch
  start stdout/stderr collectors into host-only attempt files
  bwrap command:
    host root read-only
    whole mailbox writable
    approved snapshot mounted read-only over claimed command.sh
...
#+begin_example
fake seal + fake bwrap + command longer than 40 lines
  run with yes
  -> pull then push
  -> newest ready directory becomes claimed
  -> exact command runs from world
  -> line warning is visible
  -> stdout/stderr and completed result are returned
  -> host-only approved snapshot remains

fake vi changes command.sh
  run with edit then yes
  -> edited bytes execute
...
first approved command publishes one successor
  ...
  -> first command completes and publishes its successor
